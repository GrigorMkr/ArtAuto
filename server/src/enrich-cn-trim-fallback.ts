/**
 * Fallback trim cards for CN cars (no network). Real Dongchedi detail overrides later.
 */
...
async function main() {
  ...
  for (const v of store.vehicles) {
    if (v.country !== "CN" || v.status !== "AVAILABLE" || v.foreign_price == null) continue;
  ...
  saveStore(store);
  console.log(`CN fallback trim cards: ${updated}`);
}
